...
`reference_id = f"rvp_{case_id}_{attempt_no}".lower()` — deterministic, human-traceable, and the exact string
the attribution matcher looks for in the confirming webhook.

> **Lowercased deliberately.** Probing live Test Mode showed Razorpay *stores* the reference with its original
> case but treats uniqueness and lookup **case-insensitively**, while our own `UNIQUE(reference_id)` in SQLite
> is case-*sensitive* (INC-012). Two references differing only in case would pass our constraint and then be
> refused by the provider — a confusing failure with no local trace. Emitting lowercase makes both uniqueness
> domains identical, which fixes it at the source rather than at each of the three comparison sites.
...
| `GET` | `/api/v1/audit/verify` | Bearer | `{valid, blocks, first_divergence_index, reason, head_hash}` — `head_hash` is what an external observer compares to detect tail truncation, which the chain cannot detect itself (DEC-022) |
...
| **8** | Two-phase outbox + retry + DLQ + reconciler | `tools/outbox.py`, `workers/drainer.py` | `reference_id` committed before any call — asserted by a test that inspects the outbox *from inside the provider call*. **Verified against live Razorpay Test Mode** (DEC-020) before the module was written: a duplicate `reference_id` is genuinely refused and the existing link is retrievable, so the mock models something true. Then demonstrated end-to-end live: kill between the provider call and the commit, restart, and the reconciler finishes with **one link at Razorpay, one action locally**. Backoff 0.5/2/8/30 s with ±25% jitter so a provider blip does not produce a synchronised retry spike; DLQ preserves the provider's own error and is replayable; a terminal case is never retried, because the customer may have paid while the row waited. |
| **9** | Attribution matcher + experiment arms | `services/attribution.py`, `services/experiments.py`, `core/stats.py` | Arm assignment deterministic, stable across restart/redelivery/replay, and computed from the case id **only** — anything correlated with outcome would make the lift a measurement of the split rather than the intervention, and a test asserts the function signature cannot see amount or LTV. Recovery counted only when **all six** conditions hold (§14.1); each one can only *reduce* the figure. `payment.captured` is deliberately excluded from the settling set because it fires for organic completions too. Lift with Wilson intervals on both arms, and a **written caveat on the report** when the intervals overlap (DEC-021). **No test asserts a target rupee figure** — they assert invariants: gross equals the sum of verified webhooks, control money is never counted, and a negative lift is reported as negative. Measured over the real corpus: gross ₹2,02,760 → **incremental ₹60,217**, lift 6.2%, **not significant at n=39 control**. |
| **10** | Audit chain + verifier + auth | `tools/audit.py`, `routers/audit.py`, `security/auth.py` | Canonical JSON chaining (sorted keys, fixed separators, unit-separated fields so `("ab","c")` and `("a","bc")` cannot collide). The timestamp is hashed **as `to_db_iso` stores it** — hashing `isoformat()` would make every block verify as corrupt on reload, the INC-013 shape. `/audit/verify` catches payload edits, forged `payload_hash`, edited `current_hash`, backdated timestamps, deleted and reordered blocks — each with the divergent index and a reason naming the failure; **verified by sabotage** (a stubbed verifier fails 6 tamper tests). Tail truncation is **stated as undetectable** and asserted as such (DEC-022). Bearer auth with `compare_digest`; unset token → open + `X-Auth-Mode: disabled` outside production, **refuses to start** in production (DEC-023). `verify_approval_hash` → 409, wired to approvals in Phase 11. Dev-only tamper endpoint, 403 in production (DEC-024). |
| **11** | HITL, approvals, consent, quiet hours, templates | `routers/approvals.py`, `guardrails/consent.py`, `services/scheduler.py` | Approval TTL sweeper expires stale approvals **and** their frozen cases; `reviewed_by` stays NULL on expiry, because writing `system` into a column meaning *the human who decided* would make the audit trail claim a review that never happened. Action ordering is **already-actioned → TTL → hash**, deliberately: a correct hash does not make four-hour-old information fresh (DEC-026). `reviewed_by` comes from the authenticated principal; `extra="forbid"` makes a body that names its own reviewer a 422. Rendering is **template + allowlisted slots, substituted in one pass and never re-scanned** — not `str.format`, which walks attribute expressions and would let `{x.__class__.__init__.__globals__}` reach process globals (DEC-025). Quiet-hours deferral proven **end to end** across rule → outbox → scheduler → drainer at every hour of the day, not just as a predicate — which is how **INC-016** was found: a message held at 22:00 for a case whose window closed at 03:00 would have been sent at 09:05, spending one of two permitted contacts on an unattributable link. Now cancelled by the sweeper and audited (DEC-027). All guards sabotage-verified. |
| **12a** | REST + SSE + real health probes | `routers/cases.py`, `routers/metrics.py`, `routers/dlq.py`, `routers/stream.py`, `core/provenance.py`, `services/metrics.py` | Every §20 read endpoint typed, documented and **bearer-protected — asserted against the live route table**, so a new endpoint is covered the moment it is registered. Provenance is a **type**, not a UI rule: `Figure` cannot be built without a badge and a basis, and a test walks the actual response body for anything money-shaped, so it fails when a tile is *added* (DEC-028). `/metrics/overview` cannot return gross without net. SSE is an in-process bus with **bounded queues that drop the oldest frame and report the count** (DEC-029); the subscriber record crashed every connection until INC-017. `/dlq/{id}/replay` reuses the **original** `reference_id`, so the provider prevents a double charge (DEC-030). `/health/deep` probes are real now — WAL confirmed, queue depths counted, chain verified — and the scheduler honestly reports `not_running`. OpenAPI exported to a committed `apps/web/openapi.json`. |
| **12b** | Command Center + batch runner | `apps/web/**`, `workers/batch.py` | Next.js 16 / React 19 / Tailwind 4, types generated from the committed `openapi.json` so a renamed field breaks the build rather than rendering `undefined` on stage. Razorpay-native palette; **not** their logo or wordmark — a third-party tool that looks at home in their ecosystem, not one passing itself off as first-party. Eight components render live: metrics, attribution with drawn confidence intervals, stopping rules, cost, approvals showing the `policy_applied_hash`, the SSE pipeline, the audit verifier **with its tamper buttons**, and the case table whose SOURCE column names rule-vs-model. A failed fetch renders an explicit error, **never a zero** (DEC-032) — zero is a number a viewer believes. `tasks.py batch` puts the corpus through the agent so the dashboard shows real computed figures; every settled case carries a `sim_evt_` verifier so it reports as SIMULATED and can never reach the RAZORPAY_VERIFIED tile (DEC-031). Finding the split was forced by **INC-018**, where one enum value meaning two things put control conversion at 89.7% and the lift at −60.5%. |
| **13** | Playbooks, chaos, the INC-018 fix at source | `agent/playbooks.py`, `routers/simulation.py`, `routers/briefing.py` | All four playbooks route correctly and **the subscription split is exact on all 24 cases**: 13 insufficient-funds → `MANDATE_RETRY` (the mandate is alive; a fresh link would throw away every future collection), 11 dead-mandate → `MANDATE_REAUTH` (re-presenting cannot succeed and burns a scheme re-presentation). Receivables get `INVOICE_REMINDER` on email and **can never be discounted** — that writes off a receivable on a price already agreed. `violations()` is applied to the **model's** proposal, not only ours: the LLM may argue for an action, it may not pick one the playbook forbids. `OBSERVED_NO_ACTION` added as a distinct terminal state, fixing INC-018 at source rather than only in the batch. Morning briefing with the **restraint sentence** — what the agent chose *not* to do — computed in SQL and never narrated by a model. Chaos endpoints with four faults, each stating what it breaks *and what should happen*; 403 in production. **Batch of 420 completes in 2.3 s** against the committed cache with zero API calls, and re-running produces identical numbers. |
| **14** | Real-webhook wiring via tunnel | `docs/webhooks.md`, `tools/tunnel.py` | **DONE, on real traffic.** A Cloudflare quick tunnel (no account, no card — DEC-035) exposed the local API; the URL was registered in the Razorpay dashboard; a genuine `payment_link.paid` arrived from Razorpay's Mumbai egress (`52.66.76.63`), HMAC-verified, `200 ACCEPTED`, `event_id TW3Rfq6VhWiuwC`, carrying a `reference_id` **we** issued — attribution condition 3 satisfied by a real event for the first time. Payload committed as `payment_link.paid.captured.json`, redacted, `provenance: captured_live_webhook`. Signature-mismatch causes journalled: re-serialised bodies are rejected (verified through the live tunnel), and **two 401s that meant opposite things** cost a session (INC-021) — a bad signature and a stale retry now log distinguishable messages. Getting there exposed **INC-019** (an undocumented error shape crashed the provider client out of the outbox's classification) and **INC-020** (uvicorn leaves the root logger unhandled, so every runtime `log.warning` in the app was silently discarded). |
