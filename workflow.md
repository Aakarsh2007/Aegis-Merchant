...
| **13** | Playbooks 2–4, chaos, eval harness | `workers/`, `tests/eval/` | The remaining playbooks, fault injection, the `MorningBriefing` and `ChaosPanel` components, and a distinct terminal state for *observed, never acted on* — the deeper INC-018 fix, deferred here rather than smuggled into a UI phase |
